...
<header className="bg-[var(--navy)] text-white"><div className="max-w-7xl mx-auto px-4 sm:px-5 py-4 flex justify-between items-center gap-3"><div><div className="portal-title font-bold text-lg sm:text-xl">البوابة الإلكترونية لمدارس التعليم المستمر</div><div className="text-xs sm:text-sm text-blue-100 mt-1">لوحة مدير النظام — الإدارة العامة للتعليم بنجران</div></div><button onClick={logout} className="flex gap-2 items-center bg-white/10 hover:bg-white/15 px-3 sm:px-4 py-2 rounded-xl shrink-0"><LogOut size={17}/> <span className="hidden sm:inline">خروج</span></button></div></header>
    <main className="max-w-7xl mx-auto p-3 sm:p-5 md:p-8"><div className="grid lg:grid-cols-[230px_1fr] gap-4 lg:gap-6">
      <aside className="card p-2 lg:p-3 h-fit flex lg:block overflow-x-auto lg:overflow-visible gap-1 sticky top-2 z-20">{nav.map(([key,label,Icon])=><button key={key} type="button" onClick={()=>go(key)} className={`shrink-0 whitespace-nowrap lg:w-full flex items-center gap-2 lg:gap-3 rounded-xl px-3 lg:px-4 py-3 text-right mb-0 lg:mb-1 text-sm lg:text-base ${tab===key?'bg-[var(--navy)] text-white':'hover:bg-slate-100'}`}><Icon size={18}/>{label}</button>)}</aside>
