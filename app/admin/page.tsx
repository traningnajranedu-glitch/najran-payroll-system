...
export default function AdminPage() {
  ...
  async function addSchool() {
    if (!schoolForm.school_code || !schoolForm.school_name) return;
    setBusy(true); setMessage('');
    const { error } = await sb.from('schools').insert({ ...schoolForm, is_active: true });
    if (error) setMessage('تعذر إضافة المدرسة: ' + error.message);
    else { setMessage('تمت إضافة المدرسة بنجاح.'); setSchoolForm({ school_code: '', school_name: '' }); await loadAll(); }
    setBusy(false);
  }
  ...
  return <div className="min-h-screen bg-slate-50 print:bg-white">
    <header className="bg-[var(--navy)] text-white print:hidden">
      <div className="max-w-7xl mx-auto px-5 py-4 flex items-center justify-between">
        ...
        <button onClick={logout} className="flex gap-2 items-center bg-white/10 px-4 py-2 rounded-xl"><LogOut size={17}/> خروج</button>
      </div>
    </header>
    ...
    <main className="max-w-7xl mx-auto p-5 md:p-8">
      <div className="grid lg:grid-cols-[220px_1fr] gap-6">
        <aside className="card p-3 h-fit print:hidden">
          {nav.map(([key, label, Icon]) => <button key={key} onClick={() => setTab(key)} className={`w-full flex items-center gap-3 rounded-xl px-4 py-3 text-right mb-1 ${tab === key ? 'bg-[var(--navy)] text-white' : 'hover:bg-slate-100'}`}><Icon size={18}/>{label}</button>)}
        </aside>
        ...
        <section className="space-y-6">
          ...
          {tab === 'overview' && <>
            <div className="flex items-center justify-between"><div><h1 className="text-2xl font-bold">لوحة التحكم</h1><p className="text-gray-500 mt-1">إدارة المدارس والموظفين وفترات المسيرات.</p></div><button onClick={loadAll} className="border bg-white rounded-xl p-3 print:hidden"><RefreshCw size={18}/></button></div>
            ...
            <div className="card p-6"><h2 className="font-bold text-lg mb-4">اختصار إدارة المسير</h2><div className="grid md:grid-cols-3 gap-3"><button onClick={() => setTab('schools')} className="border rounded-xl p-4 text-right hover:bg-slate-50">إضافة مدرسة جديدة</button><button onClick={() => setTab('teachers')} className="border rounded-xl p-4 text-right hover:bg-slate-50">إضافة موظفين للمدرسة</button><button onClick={() => setTab('periods')} className="border rounded-xl p-4 text-right hover:bg-slate-50">إنشاء فترة مسير جديدة</button></div></div>
          </>}
