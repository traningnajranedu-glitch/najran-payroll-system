...
import { Building2, Users, CalendarDays, CheckCircle2, ShieldCheck, LogOut, RefreshCw, Printer } from 'lucide-react';
...
export default function AdminPage() {
  ...
  const [schoolId,setSchoolId]=useState(''), [periodId,setPeriodId]=useState(''), [message,setMessage]=useState(''), [busy,setBusy]=useState(false);

  async function load(){
    setLoading(true); const {data:{user}}=await sb.auth.getUser();
    ...
    const [s,t,p]=await Promise.all([sb.from('schools').select('*').order('school_name'),sb.from('teachers').select('*').order('full_name'),sb.from('payroll_periods').select('*').order('start_date',{ascending:false})]);
    ...
    if(!schoolId&&s.data?.[0])setSchoolId(s.data[0].id);if(!periodId&&p.data?.[0])setPeriodId(p.data[0].id);setLoading(false);
  }
  async function loadRecords(){if(!periodId)return;let q=sb.from('payroll_records').select('*').eq('period_id',periodId);if(schoolId)q=q.eq('school_id',schoolId);const {data,error}=await q;if(error)setMessage(error.message);else setRecords(data||[])}
  ...
  async function generate(){if(!schoolId||!periodId)return;setBusy(true);const {error}=await sb.rpc('generate_school_payroll',{p_school_id:schoolId,p_period_id:periodId});if(error)setMessage('تعذر تجهيز المسير: '+error.message);else{setMessage('تم تجهيز مسير المدرسة بنجاح');loadRecords()}setBusy(false)}
  async function status(id:string,status:string){setBusy(true);const {error}=await sb.rpc('set_payroll_status',{p_record_id:id,p_status:status});if(error)setMessage(error.message);else loadRecords();setBusy(false)}
  async function periodState(p:Period){setBusy(true);const open=!(p.is_open&&p.allow_edit);const {error}=await sb.rpc('set_period_state',{p_period_id:p.id,p_is_open:open,p_allow_edit:open});if(error)setMessage(error.message);else load();setBusy(false)}
  ...
  const nav=[['overview','نظرة عامة',Building2],['schools','المدارس',Building2],['teachers','الموظفون',Users],['periods','فترات المسيرات',CalendarDays],['payroll','إدارة المسيرات',CheckCircle2],['manager-permissions','صلاحية مدير النظام',ShieldCheck]] as const;
  ...
  return <div dir="rtl" className="min-h-screen bg-slate-50">
    ...
    <main className="max-w-7xl mx-auto p-5 md:p-8"><div className="grid lg:grid-cols-[230px_1fr] gap-6">
      <aside className="card p-3 h-fit">{nav.map(([key,label,Icon])=><button key={key} type="button" onClick={()=>{setTab(key);if(key==='manager-permissions')location.href='/admin/school-print'}} className={`w-full flex items-center gap-3 rounded-xl px-4 py-3 text-right mb-1 ${tab===key?'bg-[var(--navy)] text-white':'hover:bg-slate-100'}`}><Icon size={18}/>{label}</button>)}</aside>
      <section className="space-y-5">{message&&<div className="bg-blue-50 text-blue-800 border border-blue-100 rounded-xl px-4 py-3">{message}</div>}
        {tab==='overview'&&<><h1 className="text-2xl font-bold">لوحة التحكم</h1><div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4"><div className="card p-5"><Building2/><b className="block text-3xl mt-3">{schools.length}</b><span className="text-gray-500">إجمالي المدارس</span></div><div className="card p-5"><Users/><b className="block text-3xl mt-3">{teachers.filter(x=>x.is_active).length}</b><span className="text-gray-500">الموظفون النشطون</span></div><div className="card p-5"><CalendarDays/><b className="block text-3xl mt-3">{periods.length}</b><span className="text-gray-500">فترات المسيرات</span></div><div className="card p-5"><CheckCircle2/><b className="block text-3xl mt-3">{records.length}</b><span className="text-gray-500">سجلات الفترة المحددة</span></div></div></>}
        {tab==='schools'&&<div className="card p-6"><h1 className="text-2xl font-bold mb-5">المدارس</h1><div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="bg-gray-50"><th className="p-3 text-right">الرمز</th><th className="p-3 text-right">اسم المدرسة</th><th className="p-3 text-right">الحالة</th></tr></thead><tbody>{schools.map(s=><tr className="border-t" key={s.id}><td className="p-3">{s.school_code}</td><td className="p-3 font-semibold">{s.school_name}</td><td className="p-3">{s.is_active?'نشطة':'موقوفة'}</td></tr>)}</tbody></table></div></div>}
        {tab==='teachers'&&<div className="card p-6"><h1 className="text-2xl font-bold mb-5">الموظفون</h1><select value={schoolId} onChange={e=>setSchoolId(e.target.value)} className="border rounded-xl px-4 py-3 mb-4 w-full"><option value="">كل المدارس</option>{schools.map(s=><option key={s.id} value={s.id}>{s.school_name}</option>)}</select><div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="bg-gray-50"><th className="p-3 text-right">الاسم</th><th className="p-3 text-right">الهوية</th><th className="p-3 text-right">الوظيفة</th><th className="p-3 text-right">التخصص</th></tr></thead><tbody>{teachers.filter(t=>!schoolId||t.school_id===schoolId).map(t=><tr className="border-t" key={t.id}><td className="p-3">{t.full_name}</td><td className="p-3">{t.national_id}</td><td className="p-3">{t.job_role}</td><td className="p-3">{t.specialization||'—'}</td></tr>)}</tbody></table></div></div>}
        ...
        {tab==='manager-permissions'&&<div className="card p-8 text-center"><ShieldCheck className="mx-auto" size={48}/><h1 className="text-2xl font-bold mt-4">صلاحية مدير النظام</h1><p className="text-gray-500 mt-2">الوصول إلى طباعة مسير كل مدرسة بشكل مستقل.</p><button onClick={()=>location.href='/admin/school-print'} className="mt-5 bg-[var(--navy)] text-white rounded-xl px-6 py-3 font-bold inline-flex items-center gap-2"><Printer size={18}/> الدخول إلى طباعة مسيرات المدارس</button></div>}
      </section>
    </div></main>
    </div>
}