  ...
  return <main dir="rtl" className="min-h-screen bg-[#f4f7fb]">
    ...
    <div className="max-w-[1500px] mx-auto p-4 sm:p-6 space-y-6">
      ...
      <div className="grid xl:grid-cols-2 gap-6">
        ...
        <ChartCard title="3. توزيع التحصيل التعليمي" sub="عدد المدارس ضمن شرائح التحصيل"><div className="h-64 flex items-end justify-around gap-4">{distribution.map((v,i)=>{const labels=['أقل من 60%','60–74%','75–89%','90% فأعلى'];return <div className="flex-1 flex flex-col items-center gap-2" key={labels[i]}><b>{v}</b><button type="button" onClick={()=>setSelectedSchool("all")} aria-label={labels[i]+" — "+v+" مدرسة"} className="w-full max-w-20 bg-indigo-600 rounded-t-xl hover:bg-indigo-700 transition-all" style={{height:Math.max(8,v*40)}}/><span className="text-xs text-slate-500 text-center">{labels[i]}</span></div>})}</div></ChartCard>
