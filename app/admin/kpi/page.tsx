...
export default function KpiDashboard(){
  ...
  const distribution=[0,0,0,0]; rows.forEach(r=>{const v=r.achievement??0;if(v>=90)distribution[3]++;else if(v>=75)distribution[2]++;else if(v>=60)distribution[1]++;else distribution[0]++;});
  const overall={
  ...
  return <main dir="rtl" className="min-h-screen bg-[#f4f7fb]">
    ...
    <div className="max-w-[1500px] mx-auto p-4 sm:p-6 space-y-6">
      ...
      <div className="grid xl:grid-cols-2 gap-6">
        <ChartCard title="1. نسبة العاملين حسب المدرسة" sub="مقارنة تفاعلية"><div className="space-y-4">{chartRows.map(r=><div key={r.school.id}><div className="flex justify-between text-xs mb-1"><b>{r.school.school_code} — {r.school.school_name}</b><span>{pct(r.staffingRate)}%</span></div><div className="h-4 bg-slate-100 rounded-full overflow-hidden"><div className="h-full bg-teal-700 rounded-full" style={{width:Math.min(100,r.staffingRate)+'%'}}/></div></div>)}</div></ChartCard>
        ...
        <ChartCard title="3. توزيع التحصيل التعليمي" sub="عدد المدارس ضمن شرائح التحصيل"><div className="h-64 flex items-end justify-around gap-4">{distribution.map((v,i)=>{const labels=['أقل من 60%','60–74%','75–89%','90% فأعلى'];return <div className="flex-1 flex flex-col items-center gap-2" key={labels[i]}><b>{v}</b><div className="w-full max-w-20 bg-indigo-600 rounded-t-xl" style={{height:Math.max(8,v*40)}}/><span className="text-xs text-slate-500 text-center">{labels[i]}</span></div>})}</div></ChartCard>
        <ChartCard title="4. النشاط مقابل التقييم" sub="مقارنة نسبة الإنجاز بجودة التقييم"><div className="space-y-4">{chartRows.map(r=><div key={r.school.id}><div className="flex justify-between text-xs mb-1"><b>{r.school.school_code}</b><span>{r.avgRating?r.avgRating.toFixed(1):'—'} / 5</span></div><div className="flex gap-2"><div className="flex-1 h-3 bg-slate-100 rounded-full"><div className="h-full bg-amber-500 rounded-full" style={{width:Math.min(100,r.activityRate)+'%'}}/></div><div className="w-28 h-3 bg-slate-100 rounded-full"><div className="h-full bg-violet-600 rounded-full" style={{width:Math.min(100,r.avgRating*20)+'%'}}/></div></div></div>)}</div></ChartCard>
      </div>
      <div className="grid xl:grid-cols-3 gap-6">
        <ChartCard title="5. بصمة المدارس متعددة المحاور" sub="مؤشرات العاملين والأنشطة والتقييم والتحصيل"><div className="grid grid-cols-2 gap-3">{chartRows.slice(0,6).map(r=><div className="border rounded-2xl p-3" key={r.school.id}><b>{r.school.school_code}</b><Mini label="العاملون" v={r.staffingRate}/><Mini label="الأنشطة" v={r.activityRate}/><Mini label="التقييم" v={r.avgRating*20}/><Mini label="التحصيل" v={r.achievement??0}/></div>)}</div></ChartCard>
        <ChartCard title="6. التحصيل مقابل المستهدف" sub="الفجوة لكل مدرسة"><div className="space-y-4">{chartRows.filter(r=>r.achievement!=null).map(r=>{const gap=(r.achievement||0)-(r.target??0);return <div key={r.school.id}><div className="flex justify-between text-xs"><b>{r.school.school_code}</b><span>{pct(r.achievement||0)}%</span></div><div className="h-4 bg-slate-100 rounded-full mt-1"><div className="h-4 bg-sky-600 rounded-full" style={{width:Math.min(100,r.achievement||0)+'%'}}/></div><small className="text-slate-500">{r.target==null?'لا يوجد مستهدف':gap>=0?'فوق المستهدف':'دون المستهدف'}{r.target!=null?' — '+Math.abs(pct(gap))+' نقطة':''}</small></div>})}</div></ChartCard>
        <ChartCard title="7. كثافة المنسوبين والأنشطة" sub="الحجم التشغيلي لكل مدرسة"><div className="space-y-3">{chartRows.map(r=><div key={r.school.id}><div className="flex justify-between text-xs"><b>{r.school.school_code}</b><span>{r.activeStaff} منسوب نشط · {r.completedActivities} تقرير</span></div><div className="h-3 bg-slate-100 rounded-full mt-1"><div className="h-full bg-emerald-600 rounded-full" style={{width:Math.min(100,r.totalStaff*4)+'%'}}/></div></div>)}</div></ChartCard>
      </div>
